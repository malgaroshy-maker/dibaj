  ...
  gridContainer.innerHTML = filtered.map(fabric => {
    ...
    return `
      <article class="catalog-card" id="catalog-card-${fabric.id}">
        <div class="card-media-wrapper" data-lightbox-product="${fabric.id}" title="انقر لمعاينة التفاصيل والأقمشة بدقة عالية">
          ...
          <button type="button" class="card-zoom-trigger" data-lightbox-product="${fabric.id}" aria-label="معاينة مكبرة للموديل ${fabric.title}" title="معاينة مكبرة للتفاصيل">
